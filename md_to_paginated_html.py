...
try:
    import markdown
    def md2html(s):
        return markdown.markdown(s, extensions=["tables", "fenced_code", "sane_lists"])
except ImportError:
...
CSS = """
@import url('https://fonts.googleapis.com/css2?family=UnifrakturMaguntia&family=IM+Fell+English:ital@0;1&family=Oswald:wght@400;700&family=Noto+Serif+SC:wght@400;700;900&display=swap');

@page { size: A4; margin: 12mm; }
body {
  font-family: "IM Fell English", "Noto Serif SC", "Songti SC", "Hiragino Sans GB", serif;
  background: #e8e8e8;
  color: #000;
  margin: 0;
  line-height: 1.6;
...
p, li, td, th { font-family: "IM Fell English", "Noto Serif SC", "Songti SC", serif; }
...
<div class="page cover">
  ...
  <p style="color:#666; margin-top:40px;">共 {len(pages)} 页 · 基于 Bare Bones Edition</p>
</div>
"""

for i, ph in enumerate(page_htmls, 1):
    # 从页内容提取"第 N 页"的 N
    m = re.search(r"第\s*(\d+)\s*页", ph)
    pn = m.group(1) if m else str(i)
    html += f'<div class="page"><span class="page-num">p. {pn}</span>\n{ph}\n</div>\n'
