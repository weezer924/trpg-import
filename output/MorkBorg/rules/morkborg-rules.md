...
12. [Optional Tables](#optional-tables)
...
1. Randomize your starting equipment on this page and the next.
...
### Starting Equipment — Container (d6)
...
### Starting Equipment — First Pack (d12)
...
### Starting Equipment — Second Pack (d12)
...
Scrolls are the twisted magic of Mörk Borg. See [Powers (Scrolls)](#powers-scrolls).

### Weapons (d10)

*Roll d6 instead of d10 if you begin with a scroll.*
...
*Roll d2 instead of d4 if you begin with a scroll.*

| d4 | Armor |
|---:|-------|
| 1 | No armor (tier 0) |
| 2 | Light (fur, padded cloth, leather etc, −d2 damage, tier 1) 20s |
| 3 | Medium armor (scale, mail etc, −d4 damage, tier 2) 100s. *dr +2 on Agility tests including defence.* |
| 4 | Heavy armor (splint, plate etc, −d6 damage, tier 3) 200s. *dr +4 on Agility tests, defence is dr+2.* |

**Shield** −1 damage 20s. You can choose to ignore all damage from one attack but the shield breaks.
...
| Salt | 4s | |
| Scissors | 9s | |
| Scroll | ~50s | worth roughly 50s to the right buyer |
| Sharp needle | 3s | |
| Waterskin | 4s | 4 days of water |

Improvised weapons do d4 damage. Who wouldn't forgo fisticuffs for a grappling hook?
...
Roll 3d6 and use the table below to generate each ability score from −3 to +3. The sum is not used in the game once the character is created, only the table value.
...
Tests are made against a difficulty rating. To succeed roll d20 ±your ability with a result equal to or greater than the dr. Creatures don't use abilities, they just roll a d20 against dr. For example, find bear trap dr14 or resist red poison dr12.
...
You can carry Strength+8 normal-sized items (for example crowbars, lard, scrolls, torches but not anvils, chests, ladders, corpses) without a problem. After that when testing Strength and Agility dr increases by 2 (from 12 to 14 etc.). It is impossible to carry more than twice Strength+8.

## Hit Points

*(PDF p.29)*

Begin with Toughness + d8. In worst case 1 hp, but never less.
...
See also: [Broken Bodies (d20)](#broken-bodies-d20) in the optional tables.
...
- **Defence:** PC takes double damage and armor is reduced one tier. When armor is damaged, penalties to Strength and Agility tests are not modified. *Armor reduced below 1st tier is ruined and cannot be repaired. For armor repair costs, see Equipment on page 24.*
...
Roll 2d6 when meeting creatures whose reaction is uncertain.
...
### Left in the debris you find (d6)
...
Roll **Presence + d4** every morning to determine how many times you can use Powers that day; choose from your available scrolls. When reading a scroll, test **Presence dr12**. If you succeed, the Power is activated and you subtract one use from your daily total. If you fail, the Power doesn't work, you lose d2 hp and you become dizzy for the next hour. During this time, Powers will always fail in the worst possible way.

The GM decides the effect on a Crit or Fumble, but see the optional table of [Arcane Catastrophes](#arcane-catastrophes).
...
| 9 | A dagger onto which the condemned carved their victims' name |
