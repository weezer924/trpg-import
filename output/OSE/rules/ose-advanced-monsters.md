...
Encounter tables are maintained in `ose-advanced-referee.md` → [Encounter Tables](./ose-advanced-referee.md#encounter-tables), which carries the canonical OSE Advanced format (Dungeon by level, Wilderness by terrain sub-tables). Monster `NA` (number appearing) values per entry are given inline in each monster stat block above.
